...
4. В модуле мендежера документа доставка реализуйте алгоритм вывода команд печати и алгоритм печати. В качестве образца можно использовать алгоритм из документа Заказ покупателя.
...
3. Убедитесь, что печетные формы формируются корректно, в том числе формируются QR-код и работает множественная печать
...
### Процесс выполнения
1. Добавьте команду печати для вывода комплекта документов в модуле мендежера документа Доставка. В качестве образца можно использовать команду вывода комплекта из документа Заказ покупателя.
...
3. Убедитесь, что печетные формы формируются корректно
...
6. Реализуйте функцию для печати договора:
    ...
    Функция НапечататьДоговорDOCX(МассивОбъектов)
        ...
        ПоместитьВоВременноеХранилище(Неопределено, АдресЛого);
...
3. Убедитесь, что печетная форма формируются корректно
